...
if [[ ! -x "${PYTHON_BIN}" ]]; then
  SHARED_PYTHON_BIN="${ROOT_DIR}/../../.venv/bin/python"
  if [[ -x "${SHARED_PYTHON_BIN}" ]]; then
...
LOG_MAX_BYTES="${EGESS_NODE_LOG_MAX_BYTES:-65536}"
LOG_MAX_BYTES_SET="0"
...
usage() {
  cat <<'EOF'
Usage: ./run_paper_eval.sh [--mode all|phase1|phase2|phase3|phase4] [--duration 60|120] [--nodes N] [--base-port PORT] [--batches N] [--batch-start N] [--start-delay-min N] [--full] [--dry-run] [--open] [--open-live] [--lean-graphs] [--proof-lite] [--log-max-kb N] [--no-node-logs] [--full-logs] [--full-evidence] [--skip-disk-check]
...
Safe logging defaults:
  ...
  Use --lean-graphs for paper collection: it disables node logs while keeping
  final dashboards and figure exports.
...
if [[ -z "${PYTHON_BIN}" ]]; then
  echo "Missing python interpreter for check-in evaluation" >&2
  exit 1
fi

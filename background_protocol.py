...
def background_protocol(config_json, node_state, state_lock, this_port, number_of_nodes, push_queue):
     ...
     #Increment the heartbeat counter to make sure nodes are alive and to track the time between heartbeats.
     node_state["heartbeat_counter"] = node_state["heartbeat_counter"] + 1
     ...
     # Below is disabled background hits
     ...
     push_queue.put(heartbeat_msg) # Add the heartbeat message to the push queue to be sent to other nodes.

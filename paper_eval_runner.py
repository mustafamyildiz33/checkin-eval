...
def _apply_action(action, events_path):
    ...
    if kind == "destroy_batch":
        for port in action.get("ports", []):
            ...
            _log_event(events_path, "state", {"label": label, "port": int(port), "sensor_state": "DESTROYED", "response": res})
        return
    ...
    for view, port in watch_ports.items():
        ...
        if protocol_state == "":
            if bool(state.get("DESTROYED", False)):
                protocol_state = "DESTROYED"
            elif bool(state.get("SURVEYING", False)):
                protocol_state = "SURVEYING"
            elif bool(state.get("ALARMED", False)):
                protocol_state = "ALARMED"
            elif bool(state.get("NORMAL", False)):
                protocol_state = "NORMAL"
            else:
                protocol_state = ""
        surveying_targets = state.get("surveying_targets", {})
        if not isinstance(surveying_targets, dict):
            surveying_targets = {}
        total_bytes = _to_int(counters.get("rx_total_bytes", 0), 0) + _to_int(counters.get("tx_total_bytes", 0), 0)
        watch_rows.append(
            {
                ...
                "total_mb": round(float(total_bytes) / 1048576.0, 3),
                "direction_label": "",
                "phase": str(protocol_state),
                "distance_hops": 99.0,
                "eta_cycles": 99.0,
                "current_missing_count": len(surveying_targets),
                "crash_sim": bool(faults.get("crash_sim", False)),
                "lie_sensor": bool(faults.get("lie_sensor", False)),
                "flap": bool(faults.get("flap", False)),
            }
